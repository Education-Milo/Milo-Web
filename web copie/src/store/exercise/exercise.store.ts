...
import type { ExerciseStore, QcmQuestion } from '@store/exercise/exercise.model';
...
export const useExerciseStore = create<ExerciseStore>((set) => ({
    ...
    post_qcm: async (lessonId: number): Promise<QcmQuestion[]> => {
        try {
            ...
            const response = await APIAxios.post(APIRoutes.POST_QCM_Lesson, null, { params :{ lesson_id: lessonId } });
