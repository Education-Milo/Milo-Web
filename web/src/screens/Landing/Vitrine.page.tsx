...
import ParentFaqCard from '@components/Landing/Parent-faq-card.component';
...
const PARENT_FAQ = [
  { q: "Est-ce que Milo fait les devoirs à sa place ?", a: "Non, Milo est un tuteur qui guide par le questionnement. Il ne donnera jamais la réponse directe, mais aidera l'enfant à cheminer vers la solution.", icon: "🎓" },
  { q: "Le contenu est-il sécurisé ?", a: "Absolument. Notre IA est bridée pour un usage strictement scolaire. Aucun échange entre utilisateurs n'est possible sur la plateforme.", icon: "🛡️" },
  { q: "Comment sont protégées les données ?", a: "Conformité RGPD totale. Nous ne vendons aucune donnée et l'anonymat de l'enfant est notre priorité absolue.", icon: "🔒" },
  { q: "Pourquoi Milo et pas une IA classique ?", a: "Milo est conçu pour la pédagogie enfantine : ton adapté, analyse des faiblesses et ludification des leçons pour un engagement maximal.", icon: "🚀" }
];
...
const VitrinePage: React.FC = () => {
  ...
  const nextFeature = () => setCurrentIndex((prev) => (prev + 1) % KIDS_FEATURES.length);
  const prevFeature = () => setCurrentIndex((prev) => (prev - 1 + KIDS_FEATURES.length) % KIDS_FEATURES.length);
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <h2 className="kids-h2">
                  {currentFeature.title.prefix} <span>{currentFeature.title.highlight}</span>
                </h2>
