...
const CircularProgress = ({ value, max, color, icon, label, sublabel }: any) => {
    ...
    return (
        <div className="graphical-card" style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
            <div className="card-decoration-icon">{icon}</div>
            <div style={{ position: 'relative', width: '90px', height: '90px', flexShrink: 0 }}>
                <svg width="90" height="90" viewBox="0 0 90 90" style={{ transform: 'rotate(-90deg)' }}>
                    <circle cx="45" cy="45" r={radius} stroke="#e2e8f0" strokeWidth="8" fill="none" />
                    <circle cx="45" cy="45" r={radius} stroke={color} strokeWidth="8" fill="none"
            ...
            <div>
                <h3 style={{ fontSize: '1.1rem', marginBottom: '0.2rem', color: '#718096', fontWeight: 600 }}>{sublabel}</h3>
                <div style={{ fontSize: '1.8rem', fontWeight: 'bold', color: '#2d3748' }}>{label}</div>
                <p style={{ color: '#a0aec0', fontSize: '0.85rem' }}>Objectif : {max}h / semaine</p>
            </div>
        </div>
    );
};
...
const getStatsForChild = (childId: string | number) => {
    ...
    if (idNum % 2 === 0) {
        ...
        return {
            ...
            recentActivity: [
                { title: 'Flashcards Anglais', score: '20 cartes • Il y a 1h', icon: '🃏', color: '#ebf8ff', textCol: '#3182ce' }
            ]
        };
    } else {
        ...
        return {
            ...
            recentActivity: [
                { title: 'Quizz Mathématiques', score: '18/20 • Il y a 2h', icon: '✅', color: '#e6fffa', textCol: '#38b2ac' },
                { title: 'Leçon importée', score: 'Histoire • Hier', icon: '📚', color: '#ebf8ff', textCol: '#3182ce' }
            ]
        };
    }
};
      ...
      <ScreenLayout>
        <div className="dashboard">
          <div className="main-column">
            ...
            <section className="welcome-section">
              <div className="welcome-content">
                <h1 className="welcome-title">{welcomeMessage || 'Bonjour !'}</h1>
                <p className="welcome-subtitle">
                    Voici les progrès récents de vos enfants.
                </p>
                
                {safeChildren.length > 0 && (
                  <div className="quick-action-buttons" style={{marginTop: '1.5rem'}}>
                    {safeChildren.map((child) => (
                      <button 
                        ...
                        style={{ 
                          background: selectedChild === child.id ? 'rgba(255, 255, 255, 0.3)' : 'rgba(255, 255, 255, 0.15)',
            ...
            {safeChildren.length === 0 && (
              <section className="section-card" style={{ textAlign: 'center', padding: '4rem 2rem' }}>
                ...
                <p style={{ color: '#718096', marginBottom: '2rem' }}>
                  Liez un compte enfant pour voir apparaître ses statistiques graphiques ici.
                </p>
                <button 
            ...
            {activeChild && statsData && (
              <>
                <section className="section-card" style={{background: 'transparent', boxShadow: 'none', padding: 0}}>
                  <div className="section-header" style={{marginBottom: '1.5rem'}}>
                    <h2 className="section-title">📊 Activité de la semaine : {activeChild.name}</h2>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                      ...
                      <CircularProgress 
                          ...
                          color={(function() {
                              const ratio = statsData.timeSpentHours / statsData.timeGoalHours;
                      ...
                      {/* Barre Dynamique */}
                      <div className="graphical-card vibrant" style={{ background: 'linear-gradient(135deg, #ff9a44, #fc6076)' }}>
                          <div className="card-decoration-icon">🎯</div>
                          <h3 style={{ fontSize: '1.1rem', marginBottom: '0.5rem', fontWeight: 600 }}>Exercices Complétés</h3>
                          
                          <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem', marginBottom: '1rem' }}>
                              <span style={{ fontSize: '2.5rem', fontWeight: 'bold' }}>{statsData.exercisesDone}</span>
                              <span style={{ opacity: 0.8 }}>/ {statsData.exercisesGoal}</span>
                          </div>
                          ...
                          <div style={{ 
                              ...
                              <div style={{ 
                                ...
                                {statsData.exercisesDone >= statsData.exercisesGoal && <span style={{fontSize: '0.8rem'}}>⭐</span>}
                              </div>
                          </div>
                      </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '2rem' }}>
                    <div style={{ flex: 1, padding: '1.5rem', background: 'rgba(72, 187, 120, 0.1)', borderRadius: '16px', borderLeft: '4px solid #48bb78' }}>
                      ...
                      <ul style={{ color: '#4a5568', paddingLeft: '1.5rem', lineHeight: '1.8' }}>
                        {statsData.strengths.map((item, index) => <li key={index}>{item}</li>)}
                      </ul>
                    </div>
                    <div style={{ flex: 1, padding: '1.5rem', background: 'rgba(239, 68, 68, 0.1)', borderRadius: '16px', borderLeft: '4px solid #ef4444' }}>
                      <h3 style={{ color: '#2d3748', marginBottom: '1rem' }}>À renforcer</h3>
                      <ul style={{ color: '#4a5568', paddingLeft: '1.5rem', lineHeight: '1.8' }}>
                        {statsData.weaknesses.map((item, index) => <li key={index}>{item}</li>)}
                      </ul>
                    </div>
                  </div>
                </section>
          ...
          <div className="sidebar-column">
            <section className="section-card graphical-card" style={{background: 'linear-gradient(to bottom right, #ffffff, #f0f4f8)'}}>
              <div className="section-header">
              ...
              {activeChild ? (
                <div style={{ textAlign: 'center', padding: '1rem 0', position: 'relative' }}>
                  ...
                  <p style={{ color: '#718096', marginBottom: '1.5rem' }}>Total accumulé</p>
                  
                  <div style={{ background: '#edf2f7', borderRadius: '20px', height: '16px', overflow: 'hidden', border: '2px solid #e2e8f0', position: 'relative' }}>
                    <div style={{ width: `${Math.min((activeChild.points || 0) / 3000 * 100, 100)}%`, background: 'linear-gradient(90deg, #ff6b35, #fbb13c)', height: '100%', borderRadius: '20px', transition: 'width 1s ease' }}></div>
                  </div>
                </div>
              ) : (
                ...
                <>
                  <div className="achievements-list">
                    {statsData.recentActivity.map((activity, index) => (
                      <div key={index} className="achievement-item">
                        <div className="achievement-icon" style={{background: activity.color, color: activity.textCol}}>{activity.icon}</div>
                  ...
                  <button 
                    style={{
                      ...
                      border: '1px solid #e2e8f0', borderRadius: '12px', fontWeight: 600, cursor: 'pointer'
                    }}
                  >
